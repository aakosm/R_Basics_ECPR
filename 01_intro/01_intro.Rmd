...
7. Some statistics - Demonstrating some possible use case for R. We will look into linear regression and hypothesis testing 
...
# First steps - Basic operations in R
...
A data frame is a rectangular data structure, where usually each row is an observation and each column is a variable. It can contain multiple types of data but columns can only contain one type. Data frames are made up of various columns. In the following we will create three objects with different data types in them. These objects are also called vectors (they have one dimension only). 
...
We will discuss each type more detail soon, but first, let's look at our data! We are using the `data.frame` function to combine the three vectors into one object, which we will call `df`. It contains the various information we have and can start using it for our analysis.
...
If we are unsatisfied with the column names that R gave based on the vector names, we can rectify this situation with the `names()` function.
```{r}
names(df) <- c("Country", "Population", "Continent")
...
TBC
NOW TALK ABOUT DIFFERENT TYPES OF DATA!!!! (and not indexing)
...
```{r}
list1 <- list(logic, country, mat1, text1)
