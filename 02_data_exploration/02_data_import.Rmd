---
title: "Session Two"
subtitle: "Importing and Exporting Data in R"
author: "Akos Mate"
date: '2019 July'
output:
    html_document:
...
Note: we use relativ paths within our project, and diligently put all of our data files into the data folder.
...
>disclaimer: I will emphasise `tidyverse` throughout the training, since it is one of the most useful meta-package: a collection of packages that helps to develop consistent workflow from data import to data cleaning to analysis. We will spend more time on this during the coming sessions.
...
We can see how life expectancy changed in Mexico, Afghanistan, Sudan and Slovenia by using the `geom_line()` geom. For this, we create a new dataset by subsetting the gapminder one. The `%in%` operator does the same thing as the `==` but for multiple values. For subsetting we use the `dplyr::filter()` function.
