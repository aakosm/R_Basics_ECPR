...
```{r eval=FALSE}
packages_s7 <- c("ggfortify", "effects", "GGally", "broom")

install.packages(packages_s7)
```
...
Let’s look at the survey data of the European Social Survey. The data file is familiar from before as we used it for some data visualization excercises. It contains a select few variables from the Hungarian survey.
...
Is p-value < 2.2e-16 small or large? Let's make sure that we avoid scientific notation in our output with the following option:
...
> Quick excercise: plot the relationship between life expectancy and GDP per capita with the `ggplot` package. No need to tinker with the plot this time. For extra, you can add a trend line with the `geom_smooth()`
